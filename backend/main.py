...
def get_git_commit() -> str:
    ...
    try:
        ...
        logger.error(f"Error reading git commit: {e}")
...
async def search_deezer_track(client: httpx.AsyncClient, title: str, artist: str) -> Optional[dict]:
    ...
    try:
        ...
        logger.error(f"Error searching Deezer for '{query}': {e}")
        return None
...
async def generate(req: GenerateRequest):
    ...
    system_prompt = (
        ...
        f"{genre_instruction}"
        "Respond ONLY with a valid JSON object containing a \"tracks\" array, where each item has \"title\" and \"artist\" fields. "
